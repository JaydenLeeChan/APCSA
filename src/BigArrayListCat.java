...
//    6.A new kitten was brought in (Angel, 3.6, 1, 25.99).  Insert it into the 2nd cell.
//    7.A new geriatric cat was found (Gimpy, 14.3, 10,  29.99). Put him on the list.
//    8.Print the updated list with a for-each loop
//    9.Replace the 3rd cat with (Sugar, 23.6, 7, 33.25) put the removed cat at the end of the list.
//    10.Switch the 2nd and 4th cats.
//    11.Print the names of the cats on the list.
//    12.Remove all cats under $26. Print the costs of each cat remaining on the list.
//    13.All cats heavier than 15 pounds need to go on a diet <--  no for-each this time.
//    14.Print the names of the cats being put on a diet.
...
//    3.The last cat has gained 10 pounds. Update the weight on the object. Print the new weight.
            cats.get(cats.size() - 1).setWeight(cats.get(cats.size() - 1).getWeight());
